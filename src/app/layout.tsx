 ...
 */
const BOOTSTRAP = `(function(){try{var ACCENTS=['emerald','ocean','violet','rose','amber','teal'];var r=document.documentElement;try{if(localStorage.getItem('em-cps-disclaimer-agreed')!=='1')r.setAttribute('data-disclaimer','pending');}catch(e){r.setAttribute('data-disclaimer','pending');}var p=JSON.parse(localStorage.getItem('em-cps-prefs')||'{}');var theme=p.theme||'light';r.setAttribute('data-theme',theme==='light'?'light':'dark');var accent=ACCENTS.indexOf(p.accent)!==-1?p.accent:'emerald';r.setAttribute('data-accent',accent);if(p.bold===true)r.setAttribute('data-weight','bold');if(p.scale)r.style.setProperty('--type-scale',String(p.scale));if(p.sidebar===true)r.classList.add('sidebar-collapsed');var meta=document.getElementById('themeColor');if(meta){meta.setAttribute('content',theme==='light'?'#ffffff':'#151d27');}}catch(e){try{document.documentElement.setAttribute('data-disclaimer','pending');}catch(_){}}})();`;
